...
//
// This exists because the ClamAV client did not. It matched the EICAR string
// locally, returned a fabricated verdict, and never opened a socket — while
// reporting itself as "clamav". Connect set a boolean. ScanFile returned Clean
// unconditionally. All of it passed the tests it shipped with, because those
// tests asserted the shape of the result rather than that any work happened.
//
...
func scannerTypes() []string { return []string{"clamav"} }
